...
def get_mikrobus_descriptor(defines):
    ...
    for key in defines:
        ...
        if key.endswith("_P_compatible_IDX_0") and val == '"zephyr,greybus-mikrobus"':
            ...
            mikrobus = MikrobusDescriptor(
                pwm, _int, rx, tx, scl, sda, mosi, miso, sck, cs, rst, an, None
            )
            break
    return mikrobus
    ...
    for key in defines:
        ...
        if key.endswith("_P_compatible_IDX_0") and val == '"zephyr,greybus-property"':
            ...
            if node + "_P_value" in defines.keys():
                value = re.sub(
                    re.compile("/\*.*?\*/", re.DOTALL), "", defines[node + "_P_value"]
                )
...
def get_cport_descriptors(defines):
    ...
    cport_keys = [
        '"zephyr,greybus-control"',
        '"zephyr,greybus-gpio-controller"',
        '"zephyr,greybus-i2c-controller"',
        '"zephyr,greybus-spi-controller"',
    ]
    cd = {}
    for key in defines:
        val = defines[key]
