...
static int greybus_control_init(const struct device *dev)
{
	...
	struct greybus_control_config *config = (struct greybus_control_config *)dev->config;
...
#define DEFINE_GREYBUS_CONTROL(_num)                                                               \
                                                                                                   \
	BUILD_ASSERT(DT_PROP(DT_PARENT(DT_DRV_INST(_num)), bundle_class) == BUNDLE_CLASS_CONTROL,  \
		     "BUNDLE_CLASS_CONTROL required");                                             \
                                                                                                   \
	BUILD_ASSERT(DT_PROP(DT_DRV_INST(_num), cport_protocol) == CPORT_PROTOCOL_CONTROL,         \
		     "CPORT_PROTOCOL_CONTROL required");                                           \
                                                                                                   \
	static const struct greybus_control_config greybus_control_config_##_num = {               \
		.id = (uint8_t)_num,                                                               \
		.bundle = (uint8_t)DT_PROP(DT_PARENT(DT_DRV_INST(_num)), id),                      \
		.bus_name = DT_NODE_FULL_NAME(DT_PARENT(DT_PARENT(DT_DRV_INST(_num)))),            \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(_num, greybus_control_init, NULL, NULL,                              \
