...
#define DELAY_US                                                                                   \
	do {                                                                                       \
		__asm volatile(DELAY_NOP DELAY_NOP DELAY_NOP DELAY_NOP DELAY_NOP);                 \
	} while (0)
...
{
	gpio_pin_configure(io_context->w1_gpio, io_context->w1_pin, GPIO_PULL_UP | GPIO_OUTPUT);
	if (bit) {
...
static inline uint8_t w1_read_bit(struct w1_io_context *io_context)
{
	...
	gpio_pin_configure(io_context->w1_gpio, io_context->w1_pin, GPIO_PULL_UP | GPIO_INPUT);
