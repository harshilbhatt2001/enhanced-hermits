...
LOG_MODULE_REGISTER(greybus_transport_tcpip_multiplex,
                    CONFIG_GREYBUS_LOG_LEVEL);
...
DNS_SD_REGISTER_TCP_SERVICE(gb_service_advertisement, CONFIG_NET_HOSTNAME,
                            "_greybuss", "local", DNS_SD_EMPTY_TXT,
                            GB_TRANSPORT_TCPIP_BASE_PORT);
#else  /* CONFIG_GREYBUS_ENABLE_TLS */
DNS_SD_REGISTER_TCP_SERVICE(gb_service_advertisement, CONFIG_NET_HOSTNAME,
                            "_greybus", "local", DNS_SD_EMPTY_TXT,
                            GB_TRANSPORT_TCPIP_BASE_PORT);
...
static struct gb_operation_hdr *gb_message_alloc(struct gb_operation_hdr *hdr) {
  struct gb_operation_hdr *msg =
      k_heap_alloc(&gb_trans_heap, hdr->size, K_NO_WAIT);
  if (!msg) {
    return NULL;
  }
...
static struct gb_message_in_transport gb_message_receive(int sock, bool *flag) {
  ...
  ret = read_data(sock, (uint8_t *)msg.msg + sizeof(hdr),
                  gb_message_payload_len(msg.msg));
...
static int gb_trans_listen_start(unsigned int cport) { return 0; }

static int gb_trans_listen_stop(unsigned int cport) { return 0; }
...
static void gb_trans_free_buf(void *ptr) { k_heap_free(&gb_trans_heap, ptr); }
...
static int netsetup() {
  ...
  if (IS_ENABLED(CONFIG_GREYBUS_ENABLE_TLS)) {
    ...
    ret = zsock_setsockopt(sock, SOL_TLS, TLS_PEER_VERIFY, &verify,
                           sizeof(verify));
    if (ret < 0) {
  ...
  LOG_INF("Greybus socket opened at port %zu",
          htons(GB_TRANSPORT_TCPIP_BASE_PORT));
...
struct gb_transport_backend *gb_transport_backend_init(size_t num_cports) {
  ...
  k_thread_create(&ctx.rx_thread, gb_trans_rx_stack,
                  K_THREAD_STACK_SIZEOF(gb_trans_rx_stack),
                  gb_trans_rx_thread_handler, NULL, NULL, NULL,
                  GB_TRANS_RX_STACK_PRIORITY, 0, K_NO_WAIT);
