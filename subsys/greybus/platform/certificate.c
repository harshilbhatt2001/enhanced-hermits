...
int greybus_tls_init(void)
{
	if (IS_ENABLED(CONFIG_GREYBUS_TLS_BUILTIN)) {
		...
		if (IS_ENABLED(CONFIG_GREYBUS_TLS_CLIENT_VERIFY_OPTIONAL) ||
			...
			LOG_DBG("Adding CA Certificate (%zu bytes)",
				sizeof(greybus_tls_builtin_ca_cert));
		...
		LOG_DBG("Adding Server Certificate (Private Key) (%zu bytes)",
			sizeof(greybus_tls_builtin_server_privkey));
		r = tls_credential_add(GB_TLS_SERVER_CERT_TAG, TLS_CREDENTIAL_PRIVATE_KEY,
				       greybus_tls_builtin_server_privkey,
				       sizeof(greybus_tls_builtin_server_privkey));
		if (r < 0) {
