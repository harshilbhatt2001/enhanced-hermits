 ...
 */
static int mikrobus_read_block(uint8_t *rdata, unsigned int count, uint16_t address,
			       struct w1_io_context *w1_io_context, struct w1_io *w1_gpio_io)
{
	int iter, found;
...
static int mikrobusid_init(const struct device *dev)
{
	...
	err = gpio_pin_configure(context->cs_gpio, config->cs_pin, config->cs_flags | GPIO_INPUT);
	if (err) {
	...
	greybus_manifest_click_fragment_clickid[mikrobusid] =
		malloc(sizeof(struct greybus_manifest_header));
	...
	mikrobus_read_block(mikrobus_manifest_start_addr_byte, MIKROBUS_ID_USER_EEPROM_SIZE,
			    MIKROBUS_ID_USER_EEPROM_ADDR, w1_io_context, w1_gpio_io);
	...
	if (mikrobus_manifest_start_addr_byte[0] > MIKROBUS_ID_EEPROM_BLOCK_PROT_CONTROL_SIZE) {
		LOG_ERR("variable manifest offset invalid [%x], reading fixed manifest",
			mikrobus_manifest_start_addr_byte[0]);
	...
	mikrobus_read_block(greybus_manifest_click_fragment_clickid[mikrobusid],
			    sizeof(struct greybus_manifest_header), mikrobus_manifest_start_addr,
			    w1_io_context, w1_gpio_io);
	LOG_HEXDUMP_DBG(greybus_manifest_click_fragment_clickid[mikrobusid],
			sizeof(struct greybus_manifest_header), "manifest header:");
	manifest_header = (struct greybus_manifest_header *)
		greybus_manifest_click_fragment_clickid[mikrobusid];
	...
	greybus_manifest_click_fragment_clickid[mikrobusid] =
		realloc(greybus_manifest_click_fragment_clickid[mikrobusid], manifest_size);
	LOG_INF("manifest read from click ID adapter, size %d bytes", manifest_size);
	mikrobus_read_block(greybus_manifest_click_fragment_clickid[mikrobusid], manifest_size,
			    mikrobus_manifest_start_addr, w1_io_context, w1_gpio_io);
	LOG_HEXDUMP_DBG(greybus_manifest_click_fragment_clickid[mikrobusid], manifest_size,
			"manifest fragment:");
...
int manifest_get_fragment_clickid(uint8_t **mnfb, size_t *mnfb_size, uint8_t id)
{
	...
	manifest_header =
		(struct greybus_manifest_header *)greybus_manifest_click_fragment_clickid[id];
...
#define DEFINE_MIKROBUS(_num)                                                                      \
	...
	static struct mikrobusid_context mikrobusid_dev_data_##_num;                               \
                                                                                                   \
	static const struct mikrobusid_config mikrobusid_config_##_num = {                         \
		...
		.rst_flags = DT_INST_GPIO_FLAGS(_num, rst_gpios),                                  \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(_num, mikrobusid_init, NULL, &mikrobusid_dev_data_##_num,            \
			      &mikrobusid_config_##_num, POST_KERNEL,                              \
