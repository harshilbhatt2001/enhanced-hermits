...
void test_greybus_setup(void)
{
	...
	zassert_not_equal(spi_dev, NULL, "failed to get device binding for " SPI_DEV_NAME);
...
static void tx_rx(const struct gb_operation_hdr *req, struct gb_operation_hdr *rsp, size_t rsp_size)
{
	...
	zassert_equal(rsp_size, r, "recv: expected: %u actual: %u", (unsigned)rsp_size, r);
...
void test_greybus_spi_protocol_version(void)
{
	...
	struct gb_spi_proto_version_response *const pv_rsp =
		(struct gb_spi_proto_version_response *)(rsp_ + sizeof(struct gb_operation_hdr));
	...
	zassert_equal(rsp->result, GB_OP_SUCCESS, "expected: GB_OP_SUCCESS actual: %u",
		      rsp->result);
...
void test_greybus_spi_master_config(void)
{
	...
	struct gb_spi_master_config_response *const master_rsp =
		(struct gb_spi_master_config_response *)(rsp_ + sizeof(struct gb_operation_hdr));
	 ...
	 * of devicetree
	 */
	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, bpw_mask),
		      sys_le32_to_cpu(master_rsp->bpw_mask), "bpw_mask: expected: %x actual: %x",
		      DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, bpw_mask),
		      sys_le32_to_cpu(master_rsp->bpw_mask));

	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, min_speed_hz),
	...
	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, max_speed_hz),
	...
	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, mode),
		      sys_le32_to_cpu(master_rsp->mode), "mode: expected: %x actual: %x",
		      DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, mode),
		      sys_le32_to_cpu(master_rsp->mode));

	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0, flags),
		      sys_le32_to_cpu(master_rsp->flags), "flags: expected: %x actual: %x",
...
void test_greybus_spi_device_config(void)
{
	...
	struct gb_spi_device_config_response *const device_rsp =
		(struct gb_spi_device_config_response *)(rsp_ + sizeof(struct gb_operation_hdr));
	 ...
	 * of devicetree
	 */
	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, mode),
		      sys_le32_to_cpu(device_rsp->mode), "mode: expected: %x actual: %x",
		      DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, mode),
		      sys_le32_to_cpu(device_rsp->mode));

	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, bpw),
		      sys_le32_to_cpu(device_rsp->bpw), "bpw: expected: %u actual: %u",
		      DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, bpw),
		      sys_le32_to_cpu(device_rsp->bpw));

	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, max_speed_hz),
		      sys_le32_to_cpu(device_rsp->max_speed_hz),
		      "max_speed_hz: expected: %u actual: %u",
		      DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, max_speed_hz),
		      sys_le32_to_cpu(device_rsp->max_speed_hz));

	zassert_equal(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, device_type),
	...
	zassert_equal(0,
		      strcmp(DT_PROP(DT_N_S_greybus0_S_gbbundle1_S_gbspi0_S_eeprom, device_name),
...
#define DESC(_speed, _len, _delay, _cs_change, _bpw, _rdwr)                                        \
	{                                                                                          \
		.speed_hz = sys_cpu_to_le32(_speed), .len = sys_cpu_to_le32(_len),                 \
		.delay_usecs = sys_cpu_to_le16(_delay), .cs_change = _cs_change,                   \
		.bits_per_word = _bpw, .rdwr = _rdwr,                                              \
	}
	...
	uint8_t req_[0 + sizeof(struct gb_operation_hdr) + sizeof(struct gb_spi_transfer_request) +
	...
	static const uint8_t write_data[] = {
		...
		0x00, AT25_WREN, AT25_WRITE,
		/* 3-byte address */
		0x00, 0x00, 0x00,
		/* 4 bytes of write data */
		0x10, 0x20, 0x30, 0x40, AT25_RDSR,
		/* 1 byte of clocked-out for read */
		0x00, AT25_READ,
		/* 3-byte address */
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 4 bytes of clocked-out for read */
	};
	...
	struct gb_spi_transfer_request *const xfer_req =
		(struct gb_spi_transfer_request *)(req_ + sizeof(*req));
	...
	struct gb_spi_transfer_response *const xfer_rsp =
		(struct gb_spi_transfer_response *)(rsp_ + sizeof(struct gb_operation_hdr));

	const uint8_t expected_data[] = {
		AT25_RDSR, 0x00, AT25_RDSR, 0x02, AT25_READ, 0x00,
	...
	zassert_equal(0, memcmp(expected_data, actual_data, sizeof(expected_data)),
